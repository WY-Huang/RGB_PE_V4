...
class RepVGGBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if deploy:
            ...
            self.rbr_identity = nn.BatchNorm2d(num_features=in_channels) if out_channels == in_channels \
...
g2_map = {i: 2 for i in optional_groupwise_layers}
g4_map = {i: 4 for i in optional_groupwise_layers}


def create_RepVGG_A0(deploy=False):
    return RepVGG(num_blocks=[2, 4, 4, 1],
...
if __name__ == "__main__":
    models = create_RepVGG_A0()
    print(models)
    ipt = torch.randn((1, 3, 224, 224))
    opt = models(ipt)
    print(opt)      # (1, 1280, 7, 7)
